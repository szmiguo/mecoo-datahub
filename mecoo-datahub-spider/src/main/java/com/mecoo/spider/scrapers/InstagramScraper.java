...
public class InstagramScraper {
    ...
    private static final Logger logger = LoggerFactory.getLogger(InstagramScraper.class);
     ...
     * @return Reels数据列表
     */
    public static List<ReelData> getUserReelsData(String username, int maxCollectedCount) {
        List<ReelData> reelsData = new ArrayList<>(maxCollectedCount);
        ...
        try {
                ...
                if (response.url().contains("/graphql/query")) {
                    try {
                        ...
                        if (isReelsDataResponse(responseBody)) {
                            logger.info("【新方法】拦截到Reels数据响应，长度: {}", responseBody.length());
                            interceptedResponses.add(responseBody);
                        }
                    } catch (Exception e) {
                        logger.warn("【新方法】读取响应失败: {}", e.getMessage());
                    }
                }
            });
            
            String reelsUrl = "https://www.instagram.com/" + username + "/reels/";
            logger.info("【新方法】导航到用户Reels页面: {}", reelsUrl);
            ...
            try {
                ...
                logger.warn("【新方法】页面加载等待异常: {}", e.getMessage());
            }
            
            logger.info("【新方法】开始收集Reels数据，目标: {} 条", maxCollectedCount);
            ...
            while (reelsData.size() < maxCollectedCount && scrollCount < maxScrolls) {
                scrollCount++;
                logger.info("【新方法】第 {} 次滚动，已收集: {} 条", scrollCount, reelsData.size());
                
                // 处理已拦截的响应
                for (String responseBody : interceptedResponses) {
                    List<ReelData> newReels = parseReelsFromResponse(responseBody);
                    for (ReelData reel : newReels) {
                        if (collectedShortCodes.add(reel.getShortCode()) && reelsData.size() < maxCollectedCount) {
                            reelsData.add(reel);
                            logger.info("成功收集第 " + reelsData.size() + " 条: " + reel.getShortCode());
                        }
                    }
                }
                ...
                if (reelsData.size() < maxCollectedCount) {
                    ...
                    logger.debug("【新方法】模拟观看内容，等待 {} 毫秒...", viewingDelay);
                    ...
                    logger.debug("【新方法】滚动后观察新内容，等待 {} 毫秒...", observeDelay);
                    ...
                    try {
                        ...
                        logger.debug("【新方法】网络请求已稳定");
                    } catch (Exception e) {
                        // 如果网络空闲等待超时，则继续等待一段时间
                        logger.debug("【新方法】网络空闲等待超时，继续等待...");
            ...
            page.close();
            logger.info("【新方法】数据收集完成，共收集到 {} 条Reels数据", reelsData.size());
    ...
    private static void performHumanLikeScroll(Page page) {
        try {
            ...
            logger.debug("执行人性化滚动: 当前位置={}, 页面高度={}, 窗口高度={}", currentScrollY, pageHeight, windowHeight);
            ...
            logger.warn("人性化滚动执行出错，使用默认滚动: {}", e.getMessage());
            // 降级到默认滚动
    ...
    private static void performProgressiveScroll(Page page, int windowHeight) {
        ...
        for (int i = 0; i < steps; i++) {
            ...
            logger.debug("第{}步滚动: {}px", (i + 1), stepDistance);
    ...
    private static void performRandomDistanceScroll(Page page, int windowHeight) {
        ...
        logger.debug("滚动距离: {}px", scrollDistance);
    ...
    private static boolean handlePageRedirect(Page page, String targetUrl) {
        try {
            ...
            if (currentUrl.equals(targetUrl)) {
                logger.info("【重定向】未发生重定向，直接访问目标页面");
                return true;
            }
            
            logger.info("【重定向】检测到页面重定向: {} -> {}", targetUrl, currentUrl);
            
            // 检查重定向类型并处理
            if (currentUrl.contains("/accounts/login")) {
                return handleLoginRedirect(page, targetUrl);
            } else if (currentUrl.contains("/challenge/")) {
                logger.warn("【重定向】检测到安全验证页面，程序终止");
                return false;
            } else if (!currentUrl.contains("instagram.com")) {
                logger.warn("【重定向】页面被重定向到非Instagram域名，程序终止");
                return false;
            } else {
                logger.info("【重定向】检测到其他类型重定向，继续执行...");
                return true;
            }
            
        } catch (Exception e) {
            logger.error("【重定向】处理重定向时发生异常: {}", e.getMessage());
            return false;
        }
    }
    ...
    private static boolean handleLoginRedirect(Page page, String targetUrl) {
        try {
            logger.info("【登录重定向】检测到登录页面，开始自动登录流程...");
            ...
            try {
                ...
                logger.info("【登录重定向】已保存登录页面截图: {}", screenshotPath);
            } catch (Exception e) {
                logger.warn("【登录重定向】截图保存失败: {}", e.getMessage());
            }
            
            // 尝试自动登录
            boolean loginSuccess = performAutoLogin(page);
            if (!loginSuccess) {
                logger.error("【登录重定向】自动登录失败");
                return false;
            }
            
            // 登录成功后重新导航到目标页面
            logger.info("【登录重定向】登录成功，重新导航到目标页面...");
            page.navigate(targetUrl);
            page.waitForTimeout(5000); // 等待页面加载
            
            // 检查是否成功到达目标页面
            String finalUrl = page.url();
            logger.info("【登录重定向】导航后的最终URL: {}", finalUrl);
            
            if (finalUrl.contains("/accounts/login")) {
                logger.error("【登录重定向】重新导航后仍在登录页面，登录验证失败");
                return false;
            }
            
            logger.info("【登录重定向】登录流程完成，成功到达目标页面");
            return true;
            
        } catch (Exception e) {
            logger.error("【登录重定向】处理登录重定向时发生异常: {}", e.getMessage());
            return false;
        }
    }
    ...
    private static boolean performAutoLogin(Page page) {
        try {
            ...
            if (!page.isVisible(passwordSelector)) {
                logger.error("【登录】未找到密码输入框");
                return false;
            }
            
            logger.info("【登录】找到登录表单，开始填写...");
            ...
            for (int i = 0; i < 10; i++) {
                String currentUrl = page.url();
                logger.debug("【登录】检查URL ({}/10): {}", (i+1), currentUrl);
                
                if (!currentUrl.contains("/accounts/login")) {
                    loginSuccess = true;
                    logger.info("【登录】登录成功，当前页面: {}", currentUrl);
                    break;
                }
            ...
            if (loginSuccess) {
                ...
                try {
                    ...
                    if (page.isVisible(saveLoginSelector)) {
                        logger.info("【登录】找到保存登录信息选项，点击保存...");
                    ...
                    page.waitForTimeout(1000);
                    if (page.isVisible(saveLoginSelector)) {
                        logger.info("【登录】再次找到保存登录信息选项，点击保存...");
                        page.click(saveLoginSelector);
                        page.waitForTimeout(2000);
                    }
                    ...
                    // 最后检查是否还有其他弹窗需要处理
                    page.waitForTimeout(1000);
                    logger.info("【登录】登录后处理完成，当前URL: {}", page.url());
                    
                } catch (Exception e) {
                    logger.warn("【登录】处理登录后步骤时出现异常: {}", e.getMessage());
                }
                
                return true;
            } else {
                logger.error("【登录】登录失败，仍在登录页面");
                return false;
            }
            
        } catch (Exception e) {
            logger.error("【登录】自动登录过程中出现异常: {}", e.getMessage());
            return false;
        }
    }
    ...
    private static void performRealisticScroll(Page page, int windowHeight) {
        ...
        for (int phase = 0; phase < 3; phase++) {
            ...
            for (int step = 0; step < phaseSteps; step++) {
                ...
                logger.debug("阶段{}步骤{}: {}px", (phase + 1), (step + 1), stepDistance);
                page.evaluate("window.scrollBy(0, " + stepDistance + ")");
     ...
     */
    public static List<ReelData> parseReelsFromResponse(String responseBody) {
        List<ReelData> reels = new ArrayList<>();
            ...
            if (response != null && 
                ...
                for (InstagramGraphQLResponse.Edge edge : response.getData().getConnection().getEdges()) {
                    if (edge.getNode() != null && edge.getNode().getMedia() != null) {
                        ReelData reel = createReelFromMedia(edge.getNode().getMedia());
                        reels.add(reel);
                    }
                }
                
                logger.info("【Gson解析】成功解析到 {} 条Reels数据", reels.size());
            }
            
        } catch (JsonSyntaxException e) {
            logger.error("【Gson解析】JSON格式错误: {}", e.getMessage());
        } catch (Exception e) {
            logger.error("【Gson解析】解析响应失败: {}", e.getMessage());
        }
     ...
     * 从媒体数据创建ReelData对象
     */
    private static ReelData createReelFromMedia(InstagramGraphQLResponse.Media media) {
        return new ReelData(
    ...
    private static void initBrowser() {
        ...
        logger.info("浏览器初始化完成，使用本地Chrome: {}", ScrapingConfiguration.getChromeExecutablePath());
        logger.info("Stealth模式已启用");
    }
    ...
    public static void startScrape() {
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            logger.info("=== Instagram 用户 Reels 数据批量提取工具 ===");
            
            String username = ScrapingConfiguration.getTargetUsername();
            logger.info("目标用户: @{}", username);

            int maxCollectedCount = ScrapingConfiguration.getMaxCollectionCount();
            logger.info("收集信息条数: {}", maxCollectedCount);

            logger.info("开始使用网络请求拦截策略获取用户 @{} 的最新{}条Reels数据...", username, maxCollectedCount);
            List<ReelData> reelsData = getUserReelsData(username, maxCollectedCount);

            logger.info("=== 数据提取结果 ===");
            if (reelsData.isEmpty()) {
                logger.warn("未能获取到任何Reels数据");
            } else {
                logger.info("成功获取到 {} 条Reels数据", reelsData.size());
                for (int i = 0; i < reelsData.size(); i++) {
                    logger.info("{}. {}", (i + 1), reelsData.get(i));
                }

            }
            //todo 保存数据库
            logger.info("数据提取完成！按回车键关闭浏览器...");
